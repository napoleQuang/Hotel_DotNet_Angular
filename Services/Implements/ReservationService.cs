namespace QLKhachSanAPI.Services.Implements
{
...

    {
        ...
        public async Task<bool> SwitchRoom(SwitchRoomVM switchroomvm)
        {
            //delete reservationroom
            var ReservationRooms = await _unitOfWork.ReservationRoomRepository.GetAsync(d => d.RoomID == switchroomvm.IdOldRoom);
            ...
            //create new reservationroom
            var reservationRoom = new ReservationRoom
            {
                RoomID = switchroomvm.IdNewRoom,
                ReservationID = switchroomvm.IdReservation
            };
            await _unitOfWork.ReservationRoomRepository.InsertAsync(reservationRoom);
            await _unitOfWork.SaveEntitiesAsync();

            //update isvalid new room and old room

            var oldRoom = await _unitOfWork.RoomRepository.GetSingleAsync(switchroomvm.IdOldRoom);
            oldRoom.IsAvaiable = true;
            await _unitOfWork.SaveEntitiesAsync();

            var newRoom = await _unitOfWork.RoomRepository.GetSingleAsync(switchroomvm.IdNewRoom);
            newRoom.IsAvaiable = false;
            await _unitOfWork.SaveEntitiesAsync();

            //get  old room type and new room type
            var oldRoomType = await _unitOfWork.RoomTypeRepository.GetSingleAsync(oldRoom.RoomTypeID);
            var newRoomType = await _unitOfWork.RoomTypeRepository.GetSingleAsync(newRoom.RoomTypeID);


            //update bill
            var reservation = await _unitOfWork.ReservationRepository.GetSingleAsync(switchroomvm.IdReservation);

            double differenceNow = (DateTime.Now - reservation.StartTime).TotalDays;
            double differenceEndTime = (reservation.EndTime - reservation.StartTime).TotalDays;

            var bill = await _unitOfWork.BillRepository.GetSingleAsync(d => (d.IDGuest == reservation.GuestID && d.Status == false));
            BillVM b = new BillVM();
            b.IdToUpdate = bill.ID;
            b.Status = bill.Status;
            b.IDGuest = bill.IDGuest;
            if (differenceNow <= 0)
            {
                b.Sum = bill.Sum - oldRoomType.DailyPrice * (differenceEndTime - 1) + newRoomType.DailyPrice * (differenceEndTime - 1);
                await _billService.UpdateBillAsync(b);
            }
            else
            {
                b.Sum = bill.Sum - oldRoomType.DailyPrice * (differenceEndTime - differenceNow) + newRoomType.DailyPrice * (differenceEndTime - differenceNow);
                await _billService.UpdateBillAsync(b);
            }
        ...
        public async Task<bool> ReserveRoomsAsync(ReservationViewModel reservationvm)  //booking
        {
            ...
            using (var transaction = _dbContext.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (checkReservation == null)
                    {
                        await _reservationHub.Clients.All.SendAsync("firsttime", await GetAllReservationsAsyncLinQ());
                    }
                    else
                    {
                        await _reservationHub.Clients.All.SendAsync("lasttime", await GetAllReservationsAsyncLinQ());
                    }


                    // Email sending task (should be background Hangfire if possible):
                    // so here, the transaction only commit when mail sent successfully !!!
                    try
                    {
                        var message = new Message(reservationvm.GuestFullName, new string[] { reservationvm.GuestEmail }, "Thanks for booking at Nexus Hotel", "Your reservation request has been sent to our staff, we will contact you as soon as possible.", null);
                        await _emailService.SendEmailAsync(message);
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine("Failed to send email to guest: ", ex);
                        return false;
                    }
