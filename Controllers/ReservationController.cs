namespace QLKhachSanAPI.Controllers
{
    ...
    public class ReservationController : Controller
    {
        ...
        /// <summary>
        /// guest hit this POST endpoint to make a reservation (involves SignalR Hub)
        /// </summary>
        [HttpPost("[action]")]
        ...
        [HttpPost("[action]")]
        public async Task<ActionResult> SwitchRoom([FromBody] SwitchRoomVM model)
        {
            try
            {
                var result = await _reservationService.SwitchRoom(model);
                if (result)
                {
                    return Ok(new { succeeded = true, message = "Switch successfully" });
                }
                return Ok(new { succeeded = false, message = "Failed to move rooms" });
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult> UpdateReservationAsync([FromBody] ReservationVM model)
        {
            try
            {
                ...
                return Ok(new { succeeded = false, message = "Failed to update Reservation! Not found RoomID or GuestID" });
            }
            catch (Exception ex)
            {
                await _unitOfWork.RollbackAsync();
